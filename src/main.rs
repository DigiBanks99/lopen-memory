...
fn handle_module(conn: &rusqlite::Connection, action: ModuleAction, json: bool) -> i32 {
    ...
    match action {
        ...
        ModuleAction::Show { module, project } => {
            ...
            if pid.is_err() {
                output::err(&pid.unwrap_err());
                return 1;
            }
            let mid = match resolve::resolve_module(conn, &module, pid.unwrap()) {
                Ok(i) => i,
                Err(e) => {
                    output::err(&e);
                    return 1;
...
fn handle_feature(conn: &rusqlite::Connection, action: FeatureAction, json: bool) -> i32 {
    ...
    match action {
        ...
        FeatureAction::Show {
            ...
            if mid.is_err() {
                output::err(&mid.unwrap_err());
                return 1;
            }
            let fid = match resolve::resolve_feature(conn, &feature, mid.unwrap()) {
                Ok(i) => i,
                Err(e) => {
                    output::err(&e);
                    return 1;
...
fn handle_task(conn: &rusqlite::Connection, action: TaskAction, json: bool) -> i32 {
    ...
    match action {
        ...
        TaskAction::Show {
            ...
            if fid.is_err() {
                output::err(&fid.unwrap_err());
                return 1;
            }
            let tid = match resolve::resolve_task(conn, &task, fid.unwrap()) {
                Ok(i) => i,
                Err(e) => {
                    output::err(&e);
                    return 1;
