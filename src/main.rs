...
fn handle_module(conn: &rusqlite::Connection, action: ModuleAction, json: bool) -> i32 {
    ...
    match action {
        ...
        ModuleAction::Show { module, project } => {
            let pid = resolve_optional_project(conn, project.as_deref());
            if pid.is_err() { output::err(&pid.unwrap_err()); return 1; }
            let mid = match resolve::resolve_module(conn, &module, pid.unwrap()) { Ok(i) => i, Err(e) => { output::err(&e); return 1; } };
...
fn handle_feature(conn: &rusqlite::Connection, action: FeatureAction, json: bool) -> i32 {
    ...
    match action {
        ...
        FeatureAction::Show { feature, module, project: _ } => {
            let mid = resolve_optional_module(conn, module.as_deref());
            if mid.is_err() { output::err(&mid.unwrap_err()); return 1; }
            let fid = match resolve::resolve_feature(conn, &feature, mid.unwrap()) { Ok(i) => i, Err(e) => { output::err(&e); return 1; } };
...
fn handle_task(conn: &rusqlite::Connection, action: TaskAction, json: bool) -> i32 {
    ...
    match action {
        ...
        TaskAction::Show { task, feature, module: _ } => {
            let fid = resolve_optional_feature(conn, feature.as_deref());
            if fid.is_err() { output::err(&fid.unwrap_err()); return 1; }
            let tid = match resolve::resolve_task(conn, &task, fid.unwrap()) { Ok(i) => i, Err(e) => { output::err(&e); return 1; } };
