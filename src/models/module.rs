...
pub fn add(conn: &Connection, project_id: i64, name: &str, description: &str, json: bool) -> i32 {
    ...
    let project_name: String = conn.query_row(
        "SELECT name FROM projects WHERE id=?1", params![project_id], |r| r.get(0)
    ).unwrap_or_default();
    let ts = now();
    match conn.execute(
        ...
        Ok(_) => {
            ...
            if json { output::print_json(&load(conn, id).map(|m| module_to_json(&m)).unwrap()); }
            else { output::print_plain(&format!("added module {}: {} (project: {})", id, name, project_name)); }
            0
        }
...
pub fn list(conn: &Connection, project_id: i64, state_filter: Option<&str>, json: bool) -> i32 {
    let modules: Vec<Module> = if let Some(s) = state_filter {
        ...
        stmt.query_map(params![project_id, s], |r| Ok(Module {
        ...
        stmt.query_map(params![project_id], |r| Ok(Module {
            id: r.get(0)?, project_id: r.get(1)?, name: r.get(2)?,
            description: r.get(3)?, details: r.get(4)?, state: r.get(5)?, last_worked_on: r.get(6)?,
        })).unwrap().filter_map(|r| r.ok()).collect()
    };
    ...
    if json {
        ...
        for m in &modules {
            println!("{:<4} {:<20} {:<12} {}", m.id, m.name, m.state, m.last_worked_on);
        }
    }
    0
}
...
pub fn show(conn: &Connection, id: i64, json: bool) -> i32 {
    let m = match load(conn, id) { Ok(m) => m, Err(e) => { output::err(&e); return 1; } };
    let project_name: String = conn.query_row(
        "SELECT name FROM projects WHERE id=?1", params![m.project_id], |r| r.get(0)
    ).unwrap_or_default();

    let mut fstmt = conn.prepare(
        "SELECT id, name, state FROM features WHERE module_id=?1 ORDER BY id"
    ).unwrap();
    ...
    if json {
        ...
        v["features"] = Value::Array(features.iter().map(|(id, name, state)| json!({"id": id, "name": name, "state": state})).collect());
...
pub fn rename(conn: &Connection, id: i64, new_name: &str, json: bool) -> i32 {
    ...
    let old = match load(conn, id) { Ok(m) => m, Err(e) => { output::err(&e); return 1; } };
    conn.execute("UPDATE modules SET name=?1, last_worked_on=?2 WHERE id=?3",
        params![new_name, now(), id]).unwrap();
    if json { output::print_json(&load(conn, id).map(|m| module_to_json(&m)).unwrap()); }
    else { output::print_plain(&format!("renamed module {}: {} → {}", id, old.name, new_name)); }
    0
}

pub fn set_description(conn: &Connection, id: i64, desc: &str, json: bool) -> i32 {
    let m = match load(conn, id) { Ok(m) => m, Err(e) => { output::err(&e); return 1; } };
    conn.execute("UPDATE modules SET description=?1, last_worked_on=?2 WHERE id=?3",
        params![desc, now(), id]).unwrap();
    if json { output::print_json(&load(conn, id).map(|m| module_to_json(&m)).unwrap()); }
    else { output::print_plain(&format!("updated description for module: {}", m.name)); }
    0
}

pub fn set_details(conn: &Connection, id: i64, details: &str, json: bool) -> i32 {
    let m = match load(conn, id) { Ok(m) => m, Err(e) => { output::err(&e); return 1; } };
    conn.execute("UPDATE modules SET details=?1, last_worked_on=?2 WHERE id=?3",
        params![details, now(), id]).unwrap();
    if json { output::print_json(&load(conn, id).map(|m| module_to_json(&m)).unwrap()); }
    else { output::print_plain(&format!("updated details for module: {}", m.name)); }
    0
}
...
pub fn transition(conn: &Connection, id: i64, to_state: &State, json: bool) -> i32 {
    ...
    match validate_transition(&m.state, to_state) {
        Err(e) => { output::err(&format!("{} for module {}", e, m.name)); return 1; }
        Ok(false) => return 0, // no-op
        Ok(true) => {}
    }
    let from = m.state.clone();
    conn.execute("UPDATE modules SET state=?1, last_worked_on=?2 WHERE id=?3",
        params![to_state.to_string(), now(), id]).unwrap();
    if json { output::print_json(&load(conn, id).map(|m| module_to_json(&m)).unwrap()); }
    else { output::print_plain(&format!("module {}: {} → {}", m.name, from, to_state)); }
    0
}
...
pub fn remove(conn: &Connection, id: i64, cascade: bool, json: bool) -> i32 {
    ...
    let count: i64 = conn.query_row(
        "SELECT COUNT(*) FROM features WHERE module_id=?1", params![id], |r| r.get(0)
    ).unwrap_or(0);
    if count > 0 && !cascade {
        output::err(&format!("module has {} feature(s); pass --cascade to remove them", count));
        return 1;
    }
    conn.execute("DELETE FROM modules WHERE id=?1", params![id]).unwrap();
    if json { output::print_json(&json!({"deleted": true, "id": id})); }
    else { output::print_plain(&format!("removed module {}: {}", id, m.name)); }
    0
}
