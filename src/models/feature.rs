...
fn module_name(conn: &Connection, module_id: i64) -> String {
    conn.query_row("SELECT name FROM modules WHERE id=?1", params![module_id], |r| r.get(0))
        .unwrap_or_default()
}
...
pub fn add(conn: &Connection, module_id: i64, name: &str, description: &str, json: bool) -> i32 {
    ...
    match conn.execute(
        ...
        Ok(_) => {
            ...
            if json { output::print_json(&load(conn, id).map(|f| feature_to_json(&f)).unwrap()); }
            else { output::print_plain(&format!("added feature {}: {} (module: {})", id, name, mname)); }
            0
        }
...
pub fn list(conn: &Connection, module_id: i64, state_filter: Option<&str>, json: bool) -> i32 {
    let features: Vec<Feature> = if let Some(s) = state_filter {
        ...
        stmt.query_map(params![module_id, s], |r| Ok(Feature {
            id: r.get(0)?, module_id: r.get(1)?, name: r.get(2)?,
            description: r.get(3)?, details: r.get(4)?, state: r.get(5)?, last_worked_on: r.get(6)?,
        ...
        stmt.query_map(params![module_id], |r| Ok(Feature {
            id: r.get(0)?, module_id: r.get(1)?, name: r.get(2)?,
            description: r.get(3)?, details: r.get(4)?, state: r.get(5)?, last_worked_on: r.get(6)?,
        })).unwrap().filter_map(|r| r.ok()).collect()
    };
    if features.is_empty() { output::print_plain("no features found"); return 0; }
    if json {
        output::print_json(&Value::Array(features.iter().map(feature_to_json).collect()));
    } else {
        for f in &features {
            println!("{:<4} {:<20} {:<12} {}", f.id, f.name, f.state, f.last_worked_on);
        }
    }
    0
}
...
pub fn show(conn: &Connection, id: i64, json: bool) -> i32 {
    let f = match load(conn, id) { Ok(f) => f, Err(e) => { output::err(&e); return 1; } };
    let mname = module_name(conn, f.module_id);

    let mut tstmt = conn.prepare(
        "SELECT id, name, state FROM tasks WHERE feature_id=?1 ORDER BY id"
    ).unwrap();
    ...
    if json {
        ...
        v["tasks"] = Value::Array(tasks.iter().map(|(id, name, state)| json!({"id": id, "name": name, "state": state})).collect());
...
pub fn rename(conn: &Connection, id: i64, new_name: &str, json: bool) -> i32 {
    ...
    let old = match load(conn, id) { Ok(f) => f, Err(e) => { output::err(&e); return 1; } };
    conn.execute("UPDATE features SET name=?1, last_worked_on=?2 WHERE id=?3",
        params![new_name, now(), id]).unwrap();
    if json { output::print_json(&load(conn, id).map(|f| feature_to_json(&f)).unwrap()); }
    else { output::print_plain(&format!("renamed feature {}: {} → {}", id, old.name, new_name)); }
    0
}

pub fn set_description(conn: &Connection, id: i64, desc: &str, json: bool) -> i32 {
    let f = match load(conn, id) { Ok(f) => f, Err(e) => { output::err(&e); return 1; } };
    conn.execute("UPDATE features SET description=?1, last_worked_on=?2 WHERE id=?3",
        params![desc, now(), id]).unwrap();
    if json { output::print_json(&load(conn, id).map(|f| feature_to_json(&f)).unwrap()); }
    else { output::print_plain(&format!("updated description for feature: {}", f.name)); }
    0
}

pub fn set_details(conn: &Connection, id: i64, details: &str, json: bool) -> i32 {
    let f = match load(conn, id) { Ok(f) => f, Err(e) => { output::err(&e); return 1; } };
    conn.execute("UPDATE features SET details=?1, last_worked_on=?2 WHERE id=?3",
        params![details, now(), id]).unwrap();
    if json { output::print_json(&load(conn, id).map(|f| feature_to_json(&f)).unwrap()); }
    else { output::print_plain(&format!("updated details for feature: {}", f.name)); }
    0
}
...
pub fn transition(conn: &Connection, id: i64, to_state: &State, json: bool) -> i32 {
    ...
    match validate_transition(&f.state, to_state) {
        Err(e) => { output::err(&format!("{} for feature {}", e, f.name)); return 1; }
        Ok(false) => return 0,
        Ok(true) => {}
    }
    let from = f.state.clone();
    conn.execute("UPDATE features SET state=?1, last_worked_on=?2 WHERE id=?3",
        params![to_state.to_string(), now(), id]).unwrap();
    if json { output::print_json(&load(conn, id).map(|f| feature_to_json(&f)).unwrap()); }
    else { output::print_plain(&format!("feature {}: {} → {}", f.name, from, to_state)); }
    0
}
...
pub fn remove(conn: &Connection, id: i64, cascade: bool, json: bool) -> i32 {
    ...
    let count: i64 = conn.query_row(
        "SELECT COUNT(*) FROM tasks WHERE feature_id=?1", params![id], |r| r.get(0)
    ).unwrap_or(0);
    if count > 0 && !cascade {
        output::err(&format!("feature has {} task(s); pass --cascade to remove them", count));
        return 1;
    }
    conn.execute("DELETE FROM features WHERE id=?1", params![id]).unwrap();
    if json { output::print_json(&json!({"deleted": true, "id": id})); }
    else { output::print_plain(&format!("removed feature {}: {}", id, f.name)); }
    0
}
